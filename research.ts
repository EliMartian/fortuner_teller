...
const APIkey: string = "WJ2S8Y8BM204TYD6"; // "TLUYLC7Y5VCCJC9A"
let globalYTDTrack: number = 0;
let globalStockRes: any; // Adjust the type according to your API response structure
let globalSPRes: any; // Adjust the type according to your API response structure
let ticker: any; // Adjust the type according to your usage
let buyCandidates: any[] = [];
let moonCandidates: any[] = [];
let pennyCandidates: any[] = [];
let superGainerCandidates: any[] = [];
let freshHotBuyCandidates: any[] = [];
let indexYTD: number = 0;

function init(): void {
    document.getElementById('new_research')!.addEventListener('click', conductNewResearch);
    document.getElementById('submission_research')!.addEventListener('click', function (el: Event) {
...
function conductNewResearch(): void {
    (document.querySelector('.new_port') as HTMLElement).style.display = 'block';
    (document.querySelector('#new_research') as HTMLElement).style.display = 'none';
}

function doResearch(): void {
    // Extract ticker symbol from the user
    ticker = (document.getElementById("research_stock_ticker") as HTMLInputElement).value;
    const URL: string = "https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol=" + ticker + "&apikey=" + APIkey;
    fetch(URL)
        .then(statusCheck)
        .then(res => res.json())
...
function calculateResearch(res: any) {
    let lastRefreshedYear: number = res['Meta Data']['3. Last Refreshed'].split('-')[0];
    ...
    globalStockRes = res;
    let updatedInfo: any = res['Weekly Adjusted Time Series'];
    ticker = ticker.toLowerCase();
    // Extract the total length of API response (in weeks)
    let length: number = Object.keys(updatedInfo).length;
    let newGraphBackground: HTMLImageElement = document.createElement('img');
    ...
    newGraphBackground.onload = function () {
        ...
        // have been avoided
        let pixelDistanceWidth: number = (((newGraphBackground.x + 452) - (newGraphBackground.x + 24)) / (length));
        ...
        let heightScaleScore: number = updatedInfo[Object.keys(updatedInfo)[0]]['5. adjusted close'] / updatedInfo[Object.keys(updatedInfo)[length - 1]]['5. adjusted close'];
        let biggestAdjCloseEverSeenForStock: number = 0;
        let lowestAdjCloseEverSeenForStock: number = 10000000000;
        // Keep track of respective intervals to print price / date information in overall stock lifetime
        let intervalCount: number = 1;
        let mostRecentClose: number = updatedInfo[Object.keys(updatedInfo)[0]]['5. adjusted close'];
        let firstEverAdjClose: number = updatedInfo[Object.keys(updatedInfo)[length - 1]]['5. adjusted close'];
        firstEverAdjClose = Number(firstEverAdjClose);
        mostRecentClose = Number(mostRecentClose);
        let y_intervals: number = 0;

        let adjCloseTotalDifference: number = mostRecentClose - firstEverAdjClose;
        let adjClosePriceInterval: number = adjCloseTotalDifference / 4;
        adjClosePriceInterval = Number(adjClosePriceInterval);

        for (let i = 0; i < length; i++) {
            let dot: HTMLSpanElement = document.createElement('span');
            dot.addEventListener('mouseover', () => {
                // Show the date of the dot when hovered over by user
                let dotDate: HTMLParagraphElement = document.createElement('p');
            ...
            dot.addEventListener('mouseout', () => {
                let currDot: any = document.getElementById('dot#' + dot.id);
            ...
            let adjustedCurrentClose: number = updatedInfo[Object.keys(updatedInfo)[i]]['5. adjusted close'];
            ...
            if (i == 0 || (i % Math.floor((length - 1) / 4) == 0)) {
                ...
                let yearLabel: HTMLSpanElement = document.createElement('span');
                let priceLabel: HTMLSpanElement = document.createElement('span');
                ...
                intervalCount++;
                let graphObj: HTMLElement = document.getElementById('graph')!;
        ...
        let dotArray: any = document.querySelectorAll('.dot');
        ...
        if (firstEverAdjClose < mostRecentClose) {
            ...
            for (let z = 0; z < dotArray.length; z++) {
                (dotArray[z] as HTMLElement).style.backgroundColor = 'green';
            }
        } else {
            // Otherwise color the dot to be red if it has since gone down in price
            for (let z = 0; z < dotArray.length; z++) {
                (dotArray[z] as HTMLElement).style.backgroundColor = 'red';
            }
        }
...
function calculateMomentum(res: any) {
    ...
    let indexTen: number = findEntry(tenOldYear, currMonth, currDay, res);
    let tenYearAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexTen]]['5. adjusted close'];

    let indexFive: number = findEntry(fiveOldYear, currMonth, currDay, res);
    let fiveYearAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexFive]]['5. adjusted close'];

    let indexStart: number = findEntry(2020, 9, 21, res);
    let startAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexStart]]['5. adjusted close'];

    let indexTwo: number = findEntry(twoOldYear, currMonth, currDay, res);
    let twoYearAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexTwo]]['5. adjusted close'];

    let indexOne: number = findEntry(oneOldYear, currMonth, currDay, res);
    let oneYearAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexOne]]['5. adjusted close'];

    indexYTD = findEntry(currYear, 1, 1, res);
    let YTDAdjClose: number = updatedInfo[Object.keys(updatedInfo)[indexYTD]]['5. adjusted close'];

    // The market price of what the security is currently going for on the stock market
    let marketPrice: number = updatedInfo[Object.keys(updatedInfo)[0]]['5. adjusted close'];
    ...
    let initialInvestmentContainer: HTMLDivElement = document.createElement('div');
    initialInvestmentContainer.classList.add("slider-container");
    let investmentLabel: HTMLLabelElement = document.createElement('label');
    investmentLabel.htmlFor = "invest_slider";
    let investmentInput: HTMLInputElement = document.createElement('input');
    ...
    let investmentText: HTMLParagraphElement = document.createElement('p');
    ...
    let YTDValue: HTMLParagraphElement = document.createElement('p');
    globalYTDTrack = ((10000 / YTDAdjClose) * marketPrice);
    calculateSP500Rating(((10000 / YTDAdjClose) * marketPrice));
    let investmentValueYTDLater: number = ((initialInvestment / YTDAdjClose) * marketPrice);
...
function displayYieldValues(initialInvestment: number, tenYearAdjClose: number, fiveYearAdjClose: number, twoYearAdjClose: number, startAdjClose: number, oneYearAdjClose: number, YTDAdjClose: number, marketPrice: number) {
    let scalerContainer: any = document.getElementById('investment_scaler_container');
    ...
    let investmentScalerContainer: HTMLDivElement = document.createElement('div');
    ...
    let investmentValueTenYearsLater: number = ((initialInvestment / tenYearAdjClose) * marketPrice);
    let investmentValueFiveYearsLater: number = ((initialInvestment / fiveYearAdjClose) * marketPrice);
    let investmentValueTwoYearsLater: number = ((initialInvestment / twoYearAdjClose) * marketPrice);
    let investmentValueStartLater: number = ((initialInvestment / startAdjClose) * marketPrice);
    let investmentValueOneYearLater: number = ((initialInvestment / oneYearAdjClose) * marketPrice);
    let investmentValueYTDLater: number = ((initialInvestment / YTDAdjClose) * marketPrice);

    // Display the value that the security would be worth today if it was purchased 
    // in a $10,000 amount 10 years ago
    let tenValue: HTMLParagraphElement = document.createElement('p');
    let tenRating: HTMLParagraphElement = document.createElement('p');
    ...
    let tenRatingScore: string = stockRatingTenYrYield(((10000 / tenYearAdjClose) * marketPrice));
    ...
    let fiveValue: HTMLParagraphElement = document.createElement('p');
    let fiveRating: HTMLParagraphElement = document.createElement('p');
    let fiveRatingScore: string = stockRatingFiveYrYield(((10000 / fiveYearAdjClose) * marketPrice));
    ...
    let startValue: HTMLParagraphElement = document.createElement('p');
    ...
    let twoValue: HTMLParagraphElement = document.createElement('p');
    ...
    let oneValue: HTMLParagraphElement = document.createElement('p');
...
function stockRatingTenYrYield(yieldValue: number): string {
    let TenYearYieldRating: string; 
...
// researching and being involved in the stock market for four years and change (no pun intended)
// Returns: A numeric letter rating of how the security did over the given period
function stockRatingFiveYrYield(yieldValue: number): string {
    let FiveYearYieldRating: string; 
    if (yieldValue >= 32500) {
...
function calculateSP500Rating(stockYTD: any) {
    let URL: string = "https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol=VOO&apikey=" + APIkey;
    fetch(URL)
        .then(statusCheck)
        .then(res => res.json())
        .then(SP500YTD)
...
function SP500YTD(res: any) {
    globalSPRes = res;
    var updatedInfo = res['Weekly Adjusted Time Series'];
    let marketPrice500: number = updatedInfo[Object.keys(updatedInfo)[0]]['5. adjusted close'];
    let YTDAdjClose500: number = updatedInfo[Object.keys(updatedInfo)[indexYTD]]['5. adjusted close'];
    let investmentValue500YTDLater: number = ((10000 / YTDAdjClose500) * marketPrice500);
    let SP500Rating: string = "";
    ...
    let SP500Txt: HTMLParagraphElement = document.createElement('p');
...
function calculatePerformanceVSSP500(res: any) {
    let updatedStockInfo: any = globalStockRes['Weekly Adjusted Time Series'];
    let updatedSPInfo: any = globalSPRes['Weekly Adjusted Time Series'];
    // Use our existing functions to figure out the oldest date the stock is on the market (of API data)
    let oldestStockDate: string = findOldestEntry(globalStockRes['Weekly Adjusted Time Series']);
    let parsedStockDate: string[] = oldestStockDate.split("-");
    let startingStockYear: number = parseInt(parsedStockDate[0]);
    ...
    let oldestSPDate: string = findOldestEntry(globalSPRes['Weekly Adjusted Time Series']);
    let parsedSPDate: string[] = oldestSPDate.split("-");
    let startingSPYear: number = parseInt(parsedSPDate[0]);
    ...
    let universalStartYear: number = Math.max(startingSPYear, startingStockYear);
    // universalStartYear = parseInt(universalStartYear);
    let universalEndYearDate: string = Object.keys(globalStockRes['Weekly Adjusted Time Series'])[0];
    let universalEndYearArray: string[] = universalEndYearDate.split("-");
    let universalEndYear: number = parseInt(universalEndYearArray[0]);
    // universalEndYear = parseInt(universalEndYear);

    let SPExceedDiv: HTMLDivElement = document.createElement('div');
    SPExceedDiv.id = 'SPExceeded';
    let SPHeading: HTMLHeadingElement = document.createElement('h3');
    SPHeading.textContent = "S&P500 exceeded Stock for the following year(s): ";
    SPExceedDiv.appendChild(SPHeading);

    let StockExceedDiv: HTMLDivElement = document.createElement('div');
    StockExceedDiv.id = 'StockExceeded';
    let StockHeading: HTMLHeadingElement = document.createElement('h3');
    ...
    let outPerf: number = 0;
    let decYear: number = 0;
    let underPGain: number = 0;
    let underPLoss: number = 0;
    let consecOutPerform: number = 0;
    let consecYearStart: number = 0;
    let consecYearArray: number[] = [];
    let yesBuy = false;
    let mostRecentBuyYear: number = 0;
    let firstEverBuyYear: number = 0;
    let recentOut: number = 0;
    let moonCandidate: boolean = false;
    let moonMultiplier = 0;
    let moonConfirmed: boolean = false;
    let pennyCandidate: boolean = false;
    let pennyConfirmed: boolean = false;
    let superGainerConfirmed: boolean = false;

    let summary: any = document.getElementById('summary');
    let momentum: any = document.getElementById('momentum');
    let overall_rating: any = document.getElementById('overall_rating');
    ...
    for (let currYear = universalStartYear; currYear < universalEndYear; currYear++) {
        ...
        let januaryStockIndex: number = findEntry(currYear, 1, 1, globalStockRes);
        // Insert 1 year gap to simulate YTD calculation
        let nextJanuaryStockIndex: number = findEntry(currYear + 1, 1, 1, globalStockRes);
        // Grab the adjusted close prices for the stock
        let januaryStockAdj: number = updatedStockInfo[Object.keys(updatedStockInfo)[januaryStockIndex]]['5. adjusted close'];
        let nextJanuaryStockAdj: number = updatedStockInfo[Object.keys(updatedStockInfo)[nextJanuaryStockIndex]]['5. adjusted close'];
        // Perform the same YTD calculation but for the S&P500
        let januarySPIndex: number = findEntry(currYear, 1, 1, globalSPRes);
        let nextJanuarySPIndex: number = findEntry(currYear + 1, 1, 1, globalSPRes);
        let januarySPAdj: number = updatedSPInfo[Object.keys(updatedSPInfo)[januarySPIndex]]['5. adjusted close'];
        let nextJanuarySPAdj: number = updatedSPInfo[Object.keys(updatedSPInfo)[nextJanuarySPIndex]]['5. adjusted close'];
        // Format the string for display to the user
        let yearString: string = `${currYear}-${currYear + 1}`;
        let yearArrayHeading: HTMLParagraphElement = document.createElement('p');
        ...
        if (nextJanuaryStockAdj / januaryStockAdj < 1) {
            ...
            if (consecYearStart == 0) {
                ...
                if ((currYear - consecYearStart <= 3) && consecOutPerform >= 3) {
                    ...
                    yesBuy = true;
                    let buyRecommendation: HTMLHeadingElement = document.createElement('h4');
                    buyRecommendation.textContent = "I would have bought this stock on January 1st of the year: " + Number(currYear + 1);
                    let yearsOutPerformed: HTMLParagraphElement = document.createElement('p');
        ...
        if (Number(universalEndYear - currYear) <= 4 && Number(januaryStockAdj) < 10) {
            if (Number(currYear + 1) == universalEndYear) {
                let januaryCurrentYearPrice: number = nextJanuaryStockAdj;
                let marketPriceCurrent: number = Number(updatedStockInfo[Object.keys(updatedStockInfo)[0]]['5. adjusted close']);
    ...
    let buyDecision: HTMLHeadingElement = document.createElement('h3');
    ...
    let final_decision: HTMLHeadingElement = document.createElement('h3');
    let justification: HTMLHeadingElement = document.createElement('h4');
    let just_year: HTMLHeadingElement = document.createElement('h4');
    let just_money: HTMLHeadingElement = document.createElement('h4');
    let overallBuyForCurrentYear: boolean = false;
    ...
    if (overallBuyForCurrentYear) {
        ...
        if (Number(mostRecentBuyYear) > 0) {
            ...
            let indexMostRecentBuy: number = findEntry(Number(mostRecentBuyYear), 1, 1, globalStockRes);
            let mostRecentBuyAdjClose: number = updatedStockInfo[Object.keys(updatedStockInfo)[indexMostRecentBuy]]['5. adjusted close'];
            let marketPriceStock: number = updatedStockInfo[Object.keys(updatedStockInfo)[0]]['5. adjusted close'];
            let calculatedValue: number = (1000 / mostRecentBuyAdjClose) * marketPriceStock;
    ...
    let hotFreshStock: HTMLHeadingElement = document.createElement('h3');
    ...
    let final_decision_greeting: HTMLHeadingElement = document.createElement('h2');
    ...
    let reset_greeting: HTMLHeadingElement = document.createElement('h2');
    reset_greeting.textContent = 'Ready to do more Research?';
    let reset_button: HTMLButtonElement = document.createElement('button');
    reset_button.addEventListener('click', function() {
        ...
        (document.getElementById('research_stock_ticker') as HTMLInputElement)?.value ?? '';
        let buyDecisions: any = document.querySelectorAll('.buy_decisions')
...
function findEntry(year: number, month: number, day: number, response: any): number {
    var updatedInfo = response['Weekly Adjusted Time Series'];
    let length: number = Object.keys(updatedInfo).length
    let minIndex: number = 0;
    let min: number = 100000;
    for (let i = 0; i < length; i++) {
        let tuple: any = Object.keys(updatedInfo)[i]; 
        let tupleData: any[] = tuple.split('-');
        if (tupleData[0] == year) {
...
function findOldestEntry(security: any): string {
    // Grab the length of the security, and return the last index of the keys (dates)
    let securityLength: number = Object.keys(security).length;
